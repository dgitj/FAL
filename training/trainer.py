...
def debiased_contrastive_loss(features, labels, temperature=0.5, beta=0.9, lambda_weight=1.0):
    """
    Computes the Debiased Contrastive Loss (DCL) which re-weights negative samples.
    
    Args:
        features (torch.Tensor): Feature vectors from the model [batch_size, feature_dim]
        labels (torch.Tensor): Ground truth labels [batch_size]
        temperature (float): Temperature parameter for scaling similarities
        beta (float): Re-weighting parameter for negatives (0 ≤ beta < 1)
        lambda_weight (float): Weight for DCL when combined with cross-entropy
        
    Returns:
        torch.Tensor: Computed DCL loss
    """
    # Handle case where features is a list (from some model architectures)
    if isinstance(features, list):
        features = features[-1]  # Use the last layer features
    ...
    # Apply exponential to scaled similarities
    exp_sim = torch.exp(sim_matrix)
    
    # Apply debiased weights to negatives
    # The weight for each negative sample is (1-beta)/(N-1) where N is batch size
    neg_weight = (1 - beta) / (batch_size - 1)
    weighted_neg_exp_sim = exp_sim * negative_mask * neg_weight
    
    # For each anchor, sum the weighted negative similarities
    neg_exp_sum = weighted_neg_exp_sim.sum(1, keepdim=True)
    
    # For each anchor, sum positive similarities
    ...
    if torch.isnan(loss).any() or torch.isinf(loss).any():
        print("[WARNING] NaN or Inf detected in debiased contrastive loss")
...
class FederatedTrainer:
    ...
    def train(self, models, criterion, optimizers, schedulers, dataloaders, num_epochs, trial_seed, val_loader=None, max_rounds=None):
        ...
        for comm in range(num_rounds):
            ...
            for epoch in range(num_epochs):
                ...
                elif self.config.LOCAL_MODEL_UPDATE == "DebiasedContrastive":
                    epoch_loss = self._train_epoch_client_debiased_contrastive(
                        selected_clients_id, models, criterion, 
                        optimizers, dataloaders, trial_seed + epoch,
                        dcl_temp=self.config.DCL_TEMPERATURE, 
                        dcl_beta=self.config.DCL_BETA,
                        dcl_lambda=self.config.DCL_LAMBDA
                    )
                else:  # KCFU
    ...
    def _train_epoch_client_debiased_contrastive(self, selected_clients_id, models, criterion, 
                                               optimizers, dataloaders, trial_seed,
                                               dcl_temp=0.5, dcl_beta=0.9, dcl_lambda=1.0):
        """
        Train client models using cross-entropy combined with debiased contrastive loss.
        ...
        Args:
            ...
            dcl_temp (float): Temperature parameter for DCL
            dcl_beta (float): Beta parameter for negative sample re-weighting
            dcl_lambda (float): Weight for DCL when combined with cross-entropy
        ...
        for c in selected_clients_id:
            ...
            for batch_idx, data in enumerate(dataloaders['train-private'][c]):
                ...
                dcl_loss = debiased_contrastive_loss(
                    features, labels, 
                    temperature=dcl_temp, 
                    beta=dcl_beta,
                    lambda_weight=dcl_lambda
                )
                
                # Combined loss
                loss = ce_loss + dcl_loss
                ...
                if batch_idx % 100 == 0:
                    with torch.no_grad():
                        ...
                        accuracy = 100 * correct / labels.size(0)
                        print(f"[Debug] Client {c} | Batch {batch_idx} | CE Loss: {ce_loss.item():.4f} | "
                              f"DCL Loss: {dcl_loss.item():.4f} | Total Loss: {loss.item():.4f} | "
                              f"Batch Accuracy: {accuracy:.2f}%")
