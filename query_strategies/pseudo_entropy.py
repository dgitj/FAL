...
class PseudoClassBalancedEntropySampler:
    ...
    def pseudo_label_data(self, model, unlabeled_loader):
        ...
        """
        print(f"[PseudoEntropy] Assigning pseudo-labels to unlabeled data using local model")
        ...
        for cls in range(config.NUM_CLASSES):  # Use NUM_CLASSES from config
            ...
            print(f"  Class {cls}: {count} samples ({percentage:.1f}%), avg entropy: {np.mean(entropy_scores[pseudo_labels == cls]) if count > 0 else 0:.3f}")
