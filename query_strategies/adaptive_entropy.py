...
class AdaptiveEntropySampler:
    def __init__(self, device="cuda"):
        """
        Initializes the Adaptive Entropy sampler.
        This sampler uses both global and local models to select samples, adapting
        its strategy based on class variance across clients.
        ...
        if device == "cuda" and not torch.cuda.is_available():
            print("[AdaptiveEntropy] CUDA not available, falling back to CPU")
            self.device = "cpu"
        else:
            self.device = device
        ...
        print(f"[AdaptiveEntropy] Using device: {self.device}")
    ...
    def calculate_local_class_distribution(self, labeled_set, dataset):
        ...
        print("[AdaptiveEntropy] Local class distribution from true labels:")
    ...
    def compute_target_counts(self, current_distribution, num_samples, labeled_set_size, available_classes):
        ...
        if self.global_class_distribution is None:
            raise ValueError("[AdaptiveEntropy] Error: Global class distribution not available")
        
        if self.debug:
            print(f"[AdaptiveEntropy] Planning to select {num_samples} samples")
            print(f"[AdaptiveEntropy] Future labeled set size will be: {future_size}")
            print(f"[AdaptiveEntropy] Available classes in unlabeled pool: {sorted(list(available_classes))}")  
        ...
        if remaining > 0:
            if self.debug:
                print(f"[AdaptiveEntropy] Distributing {remaining} remaining samples to maximize balance")
        ...
        if final_count != num_samples:
            print(f"[AdaptiveEntropy] WARNING: Target count {final_count} doesn't match budget {num_samples}")
        ...
        if self.debug:
            print(f"[AdaptiveEntropy] Target counts per class: {target_counts}")
            print(f"[AdaptiveEntropy] Total samples to select: {sum(target_counts.values())}")
    ...
    def pseudo_label_data(self, model, unlabeled_loader):
        ...
        """
        print(f"[AdaptiveEntropy] Assigning pseudo-labels to unlabeled data")
        ...
        print(f"[AdaptiveEntropy] Pseudo-label distribution:")
        for cls in range(config.NUM_CLASSES):  # Use NUM_CLASSES from config
    ...
    def select_samples(self, model, model_server, unlabeled_loader, client_id, unlabeled_set, num_samples, 
                    labeled_set=None, seed=None, global_class_distribution=None, 
                    class_variance_stats=None, current_round=0, total_rounds=5):
        """
        Selects samples using adaptive strategy based on class variance.
        ...
        Args:
            ...
            class_variance_stats (dict, optional): Statistics about class variance across clients.
        ...
        """
        # Calculate adaptation factor (0 to 1) that increases with rounds
        adaptation_factor = min(1.0, current_round / (total_rounds * 0.6))  # Full adaptation by 60% of rounds
        
        if self.debug:
            print(f"\n[AdaptiveEntropy] Client {client_id}: Selecting {num_samples} samples")
            print(f"[AdaptiveEntropy] Unlabeled pool size: {len(unlabeled_set)}")
            print(f"[AdaptiveEntropy] Round {current_round+1}/{total_rounds}, Adaptation factor: {adaptation_factor:.2f}")
        ...
        if labeled_set is not None:
            print(f"[AdaptiveEntropy] Using provided labeled set with {len(labeled_set)} samples")
        else:
            labeled_set = []
            print(f"[AdaptiveEntropy] No labeled set provided, assuming empty labeled set")
        ...
        print(f"[AdaptiveEntropy] Cycle {self.client_cycles.get(client_id, 0)} - Calculating local class distribution")
        ...
        if global_class_distribution is not None:
            ...
            print("[AdaptiveEntropy] Using global class distribution from trainer")
            
            # Print the global distribution
            print("[AdaptiveEntropy] Global class distribution:")
            for cls in range(config.NUM_CLASSES):
            ...
            raise ValueError("[AdaptiveEntropy] Error: Global class distribution not provided. Cannot proceed without global distribution.")
        
        # Get labeled sample class distribution using pseudo-labels from the server model
        ...
        if total_labeled > 0:
            ...
            print(f"[AdaptiveEntropy] Labeled set pseudo-class distribution:")
            for cls in range(config.NUM_CLASSES):  # Use NUM_CLASSES from config
        ...
        # Generate pseudo-labels for unlabeled set using server model
        server_indices, server_labels, server_confidence, server_entropy = self.pseudo_label_data(
            model=model_server,  # Use server model for global perspective
            unlabeled_loader=unlabeled_loader
        )
        
        # Get all available pseudo-classes in the unlabeled pool
        available_classes = set(server_labels)
        print(f"[AdaptiveEntropy] Available pseudo-classes in unlabeled pool: {sorted(list(available_classes))}")
        
        # Identify high-variance and low-variance classes if variance stats are provided
        high_variance_classes = []
        low_variance_classes = []
        
        if class_variance_stats is not None and 'class_stats' in class_variance_stats:
            # Get class statistics
            class_stats = class_variance_stats['class_stats']
            
            # Define threshold for high variance (use median CV as threshold)
            cv_values = [stats['cv'] for cls, stats in class_stats.items() 
                        if cls in available_classes and not np.isinf(stats['cv'])]
            if cv_values:
                cv_threshold = np.median(cv_values)
                
                # Identify high and low variance classes
                high_variance_classes = [cls for cls in available_classes 
                                        if cls in class_stats and class_stats[cls]['cv'] > cv_threshold]
                low_variance_classes = [cls for cls in available_classes 
                                    if cls in class_stats and class_stats[cls]['cv'] <= cv_threshold]
                
                if self.debug:
                    print(f"[AdaptiveEntropy] CV threshold: {cv_threshold:.2f}%")
                    print(f"[AdaptiveEntropy] High variance classes: {high_variance_classes}")
                    print(f"[AdaptiveEntropy] Low variance classes: {low_variance_classes}")
            
        # Fallback if variance stats are not available or processing failed
        if not high_variance_classes and not low_variance_classes:
            # Consider all classes as low-variance (use confidence-based selection)
            low_variance_classes = list(available_classes)
            if self.debug:
                print(f"[AdaptiveEntropy] No variance data available, treating all classes as low-variance")
        
        # Calculate the target counts for each class to achieve a better balance
        ...
        # If adaptation factor is high enough, also generate predictions with local model
        local_entropy = None
        local_indices = None
        local_labels = None
        local_confidence = None
        
        if adaptation_factor > 0.2 and high_variance_classes:  # Only if we have high-variance classes
            local_indices, local_labels, local_confidence, local_entropy = self.pseudo_label_data(
                model=model,  # Use local model for client-specific perspective
                unlabeled_loader=unlabeled_loader
            )
            
            # Calculate agreement between server and local model
            if self.debug:
                agreement = sum(1 for s, l in zip(server_labels, local_labels) if s == l)
                agreement_rate = agreement / len(server_labels) if len(server_labels) > 0 else 0
                print(f"[AdaptiveEntropy] Server-Local model agreement rate: {agreement_rate:.3f}")
        
        # Organize samples by pseudo-class with their scores
        class_samples = {cls: [] for cls in range(config.NUM_CLASSES)}
        
        # Group samples by class with relevant scores
        for i, (idx, label, conf, ent) in enumerate(zip(
            server_indices, server_labels, server_confidence, server_entropy)):
            
            if local_entropy is not None:
                # Include local model entropy when in adaptation phase
                local_ent = local_entropy[i]
                sample_data = (idx, conf, ent, local_ent)
            else:
                # Otherwise just use server model metrics
                sample_data = (idx, conf, ent, 0)  # Use 0 placeholder for local entropy
                
            class_samples[label].append(sample_data)
        
        # Select samples for each class with adaptive strategy
        selected_samples = []
        balanced_selections = {}
        ...
        for cls in sorted_classes:
            ...
            if cls not in class_samples or len(class_samples[cls]) == 0:
                if self.debug:
                    print(f"[AdaptiveEntropy] No unlabeled samples available for pseudo-class {cls}")
                continue
            
            samples = class_samples[cls]
            is_high_variance = cls in high_variance_classes
            
            # Apply adaptive selection strategy based on variance and round
            if adaptation_factor > 0.2 and local_entropy is not None:  # Only adapt after 20% of rounds and if local predictions available
                if is_high_variance:
                    # For high-variance classes: use LOCAL MODEL UNCERTAINTY (entropy)
                    # Sort by local entropy (highest first) - higher entropy = more uncertainty
                    samples.sort(key=lambda x: x[3], reverse=True)
                    
                    if self.debug:
                        print(f"[AdaptiveEntropy] Class {cls}: HIGH variance - Using local entropy with weight {adaptation_factor:.2f}")
                else:
                    # For low-variance classes: use GLOBAL MODEL CONFIDENCE
                    samples.sort(key=lambda x: x[1], reverse=True)  # Highest confidence first
                    
                    if self.debug:
                        print(f"[AdaptiveEntropy] Class {cls}: LOW variance - Using global confidence")
            else:
                # Early rounds: use global model confidence for all classes
                samples.sort(key=lambda x: x[1], reverse=True)  # Highest confidence first
            
            # Select samples according to our sorting
            num_to_select = min(target_counts[cls], len(samples))
            selected_indices = [sample[0] for sample in samples[:num_to_select]]
            selected_samples.extend(selected_indices)
            
            balanced_selections[cls] = num_to_select
            if self.debug:
                print(f"[AdaptiveEntropy] Selected {num_to_select} samples from pseudo-class {cls}")
        
        # Handle any remaining budget
        remaining_to_select = num_samples - len(selected_samples)
        if remaining_to_select > 0:
            if self.debug:
                print(f"[AdaptiveEntropy] Still need to select {remaining_to_select} additional samples")
            
            # As a last resort, select any remaining samples using adaptive strategy
            remaining_indices = [idx for idx in unlabeled_set if idx not in selected_samples]
            if remaining_indices:
                # Map indices to scores
                remaining_scores = {}
                
                for i, idx in enumerate(server_indices):
                    if idx in remaining_indices:
                        cls = server_labels[i]
                        is_high_var = cls in high_variance_classes
                        
                        if adaptation_factor > 0.2 and local_entropy is not None and is_high_var:
                            # For high-variance classes, use local entropy as score
                            score = local_entropy[i]  # Higher is better
                        else:
                            # For low-variance classes, use server confidence
                            score = server_confidence[i]  # Higher is better
                            
                        remaining_scores[idx] = score
                ...
                if self.debug:
                    print(f"[AdaptiveEntropy] Selected {additional} additional samples using adaptive scoring")
        ...
        for idx in selected_samples:
            found = False
            for i, original_idx in enumerate(server_indices):
                if original_idx == idx:
                    selected_pseudo_classes.append(server_labels[i])
                    found = True
                    break
            if not found:
        ...
        print(f"\n[AdaptiveEntropy] Final selection pseudo-class distribution:")
        for cls in range(config.NUM_CLASSES):  # Use NUM_CLASSES from config
        ...
        print(f"[AdaptiveEntropy] Total selected: {len(selected_samples)} out of budget {num_samples}")
        ...
        dist_error = sum(abs(future_distribution.get(cls, 0) - self.global_class_distribution.get(cls, 0)) 
                        for cls in range(config.NUM_CLASSES)) / 2
        print(f"[AdaptiveEntropy] Distribution error after selection: {dist_error:.4f}")
        ...
        if total_labeled > 0:
            ...
            initial_error = sum(abs(initial_distribution.get(cls, 0) - self.global_class_distribution.get(cls, 0)) 
                            for cls in range(config.NUM_CLASSES)) / 2
            improvement = initial_error - dist_error
            print(f"[AdaptiveEntropy] Initial error: {initial_error:.4f}, Improvement: {improvement:.4f}")
            if initial_error > 0:
                print(f"[AdaptiveEntropy] Relative improvement: {improvement/initial_error*100:.1f}% better")
